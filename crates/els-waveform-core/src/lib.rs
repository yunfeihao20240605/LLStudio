...
impl WaveformEngine for FfmpegWaveformEngine {
    fn generate(&self, audio: &AudioSource) -> els_types::AppResult<WaveformData> {
        ...
        Ok(WaveformData {
            duration_secs,
            bins: generate_demo_bins(duration_secs, audio.quality.target_bins_for_duration(duration_secs)),
        })
    }
    ...
    F: FnMut(&[WaveformBin], usize),
{
    ...
    let mut bins = vec![WaveformBin { min: 1.0, max: -1.0 }; target_bins];
    ...
    loop {
        let read = stdout.read(&mut buffer).map_err(|err| {
            els_types::AppError::Io(format!("failed reading ffmpeg preview waveform output: {err}"))
        })?;
        if read == 0 {
            break;
        }
        ...
        for chunk in carry[..complete_len].chunks_exact(4) {
            ...
            let bin_index =
                (sample_index.saturating_mul(target_bins) / expected_samples.max(1)).min(target_bins - 1);
...
fn ffmpeg_cli_path() -> PathBuf {
    ...
    for candidate in ["/opt/local/bin/ffmpeg", "/usr/local/bin/ffmpeg", "/opt/homebrew/bin/ffmpeg"] {
    ...
    F: FnMut(&[WaveformBin], usize),
{
    ...
    let context =
        ffmpeg::codec::context::Context::from_parameters(stream.parameters()).map_err(ffmpeg_error)?;
...
fn decode_i16(bytes: &[u8], index: usize) -> f32 {
    decode_fixed_bytes::<2>(bytes, index, |value| i16::from_le_bytes(value) as f32 / 32768.0)
}
...
fn decode_f64(bytes: &[u8], index: usize) -> f32 {
    decode_fixed_bytes::<8>(bytes, index, |value| f64::from_le_bytes(value) as f32)
        .clamp(-1.0, 1.0)
}
...
fn read_f64(bytes: &[u8], cursor: &mut usize) -> els_types::AppResult<f64> {
    let value = bytes
        .get(*cursor..(*cursor + 8))
        .ok_or_else(|| els_types::AppError::Io("waveform cache truncated while reading f64".into()))?;
    let mut raw = [0_u8; 8];
...
fn read_f32(bytes: &[u8], cursor: &mut usize) -> els_types::AppResult<f32> {
    let value = bytes
        .get(*cursor..(*cursor + 4))
        .ok_or_else(|| els_types::AppError::Io("waveform cache truncated while reading f32".into()))?;
    let mut raw = [0_u8; 4];
    raw.copy_from_slice(value);
    *cursor += 4;
...
fn read_u32(bytes: &[u8], cursor: &mut usize) -> els_types::AppResult<u32> {
    let value = bytes
        .get(*cursor..(*cursor + 4))
        .ok_or_else(|| els_types::AppError::Io("waveform cache truncated while reading u32".into()))?;
    let mut raw = [0_u8; 4];
    raw.copy_from_slice(value);
    *cursor += 4;
...
fn cache_file_path(video_path: &str, quality: WaveformQuality) -> els_types::AppResult<PathBuf> {
    ...
    let metadata = fs::metadata(video_path).map_err(|err| {
        els_types::AppError::Io(format!("failed to stat video file for waveform cache: {err}"))
    })?;
    ...
    Ok(cache_dir.join(format!("{:016x}-{}.waveform", hasher.finish(), quality.tag())))
}
...
mod tests {
    use super::{
        AudioSource, FfmpegWaveformEngine, WAVEFORM_CACHE_MAGIC, WaveformBin, WaveformEngine,
        WaveformQuality, cache_file_path, load_waveform_cache, save_waveform_cache,
    };
    ...
    fn generates_non_empty_demo_waveform() {
        ...
        let data = engine.generate(&AudioSource::default()).expect("generate waveform");
    ...
    fn preview_progress_reports_multiple_chunks() {
        ...
        assert_eq!(progress.last().map(|(_, loaded)| *loaded), Some(data.bins.len()));
    ...
    fn preview_bins_scale_with_duration() {
        assert_eq!(WaveformQuality::Preview.target_bins_for_duration(60.0), 1_024);
        assert_eq!(WaveformQuality::Preview.target_bins_for_duration(3_600.0), 36_032);
        assert_eq!(WaveformQuality::Preview.target_bins_for_duration(6_600.0), 66_048);
        assert_eq!(WaveformQuality::Preview.target_bins_for_duration(20_000.0), 131_072);
    }
}
